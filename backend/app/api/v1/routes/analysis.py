...
from app.schemas.analysis import AnalyzeRequest, DataFrameFollowupRequest, OFATRequest, PlotFollowupRequest
...
@router.post("/ofat", response_model=dict)
async def run_ofat(
    body: OFATRequest,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """OFAT 일관성 분석 실행 (비동기 작업)"""
    session_id = UUID(body.session_id)
    branch_id = UUID(body.branch_id)
    ...
    if body.source_artifact_id:
        ...
        if not artifact or not artifact.file_path:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=error_response(ErrorCode.DATASET_NOT_FOUND, "선택한 데이터프레임을 찾을 수 없습니다."),
            )
        ...
        if not session.active_dataset_id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=error_response(ErrorCode.NO_ACTIVE_DATASET, ERROR_MESSAGES[ErrorCode.NO_ACTIVE_DATASET]),
            )
    ...
    try:
        from app.worker.tasks import run_ofat_task
        from app.worker.queue import enqueue_job
        ...
        logger.error("OFAT 작업 큐 제출 실패", error=str(e))
        from app.db.models.job import JobStatus as JS
        job_run.status = JS.failed
        job_run.error_message = f"작업 큐 제출 실패: {str(e)}"
        db.add(job_run)
        await db.flush()
